...
echo "--- BEGIN rd_cancel__tib_cancel ---"
docker-compose exec --user "0:0" --privileged docdel bundle exec rails runner integration_test_scripts/rd_cancel__tib_cancel.rb $ORDER_ID $EXTERNAL_RD_ORDER_NUMBER $EXTERNAL_TIB_ORDER_NUMBER
if [[ $? != 0 ]]; then
	echo "Simulation of cancellation of TIB order in Docdel failed."
	exit 2
fi
echo "--- END rd_cancel__tib_cancel ---"
